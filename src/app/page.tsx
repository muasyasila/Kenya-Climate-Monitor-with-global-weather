...
import {
  ...
  Activity, Wind, Droplets, Thermometer, Gauge,
  AlertCircle, CheckCircle, ChevronDown, Star, ArrowLeftRight, FileText, Globe
} from 'lucide-react';
import { MetricCard } from '@/components/MetricCard';
import { WeatherChart } from '@/components/WeatherChart';
import { formatDate, formatTime, getAQICategory, getWindDirection, downloadJSON, downloadCSV } from '@/lib/utils';
import { useKeyboardShortcuts } from '@/hooks/useKeyboardShortcuts';
import { exportToPDF } from '@/lib/exportToPDF';
import { HistoricalChart } from '@/components/HistoricalChart';
import { WeatherGlobe } from '@/components/WeatherGlobe';
import { GlobalWeatherDetail } from '@/components/GlobalWeatherDetail';
...
export default function Home() {
  ...
  const [globalLocation, setGlobalLocation] = useState<{ name: string; lat: number; lon: number } | null>(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Main Kenya content - wrapped for PDF export */}
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <MetricCard
                  title="Wind"
                  value={Math.round(currentData.current.wind_speed)}
                  unit="km/h"
                  icon={Wind}
                  subtitle={`Direction: ${getWindDirection(currentData.current.wind_direction)}`}
                />
                <MetricCard
                  title="Pressure"
                  value={Math.round(currentData.current.pressure)}
                  unit="hPa"
                  icon={Gauge}
                />
                <MetricCard
                  title="Feels Like"
                  value={Math.round(currentData.current.temperature)}
                  unit="°C"
                  icon={Thermometer}
                />
              </div>
        ...
        {/* ===== GLOBAL EXPLORER SECTION (Always visible, unavoidable) ===== */}
        <div className="mt-16 pt-8 border-t-2 border-gray-200">
          <div className="flex items-center justify-between mb-6">
            ...
            {globalLocation && (
              <button
                onClick={() => setGlobalLocation(null)}
          ...
          {globalLocation ? (
            <GlobalWeatherDetail
              cityName={globalLocation.name}
              lat={globalLocation.lat}
              lon={globalLocation.lon}
              onBack={() => setGlobalLocation(null)}
            />
          ) : (
            <WeatherGlobe
              onCitySelect={(city: string) => {
                // Only handle Kenyan cities to switch focus
                if (AVAILABLE_CITIES.includes(city as any)) {
                  setSelectedCity(city);
                  router.push(`/?city=${encodeURIComponent(city)}`, { scroll: false });
                  // Scroll back to top to show Kenya section
